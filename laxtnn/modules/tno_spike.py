...
class TnoSpike(Tno):
    def __init__(self, h, dim, *args, spike_len=32, **kwargs):

        super(TnoSpike, self).__init__(h, dim, *args, **kwargs)

        self.forward_tno = (
            self.forward_causal if self.causal else self.forward_non_causal
        )

        self.pad = nn.ConstantPad1d([spike_len - 1, 0], 0.0)
    ...
    def forward(self, x, dim=-2, normalize=False):
        ...
        spike_input = self.pad(x.squeeze(1).transpose(2, 1))
        spike_output = self.spike(spike_input).transpose(2, 1).unsqueeze(1)

        return self.forward_tno(x, dim=dim, normalize=normalize) + spike_output


# class TnoSpike(nn.Module):
#     def __init__(
#         self,
#         h,
#         dim,
...
#         self.dim = dim
#         self.causal = causal
#         self.par_type = par_type  # par_type
#         self.zero_value = 0
...
#         self.pad = nn.ConstantPad1d([0, spike_len - 1], 0.0)
#         self.spike = nn.Conv1d(
#             dim,
#             dim,
#             kernel_size=spike_len,
